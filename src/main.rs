...
fn main() {
    ...
    ((Z1, A1), (Z2, A2)) = forward_propogation(&W1, &B1, &W2, &B2, x_train.clone());

    ((delta_W1, delta_B1), (delta_W2, delta_B2)) =
        back_propogation(Z1, A1, A2, W2, x_train.clone(), y_train.clone());
...
fn ReLU_Derivative(Z: &mut Matrix) -> &Matrix {
    for element in Z.iter_mut() {
        if element > &mut 0.0 {
            *element = 1.0;
        } else {
...
fn back_propogation(
    ...
    let delta_Z1: Matrix =
        W2.reversed_axes().dot(&difference_pred_actual) * ReLU_Derivative(&mut Z1);
